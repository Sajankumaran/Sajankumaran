<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f2027,50:203a43,100:00ff9d&height=230&section=header&text=M%20U%20Sajan%20Kumaran&fontSize=46&fontColor=ffffff&animation=fadeIn&fontAlignY=36&desc=Web%20Developer%20%7C%20Cybersecurity%20Enthusiast%20%7C%20Future%20Blue%20Team%20Operator&descAlignY=56&descSize=17" width="100%"/>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=23&duration=2800&pause=800&color=00FF9D&center=true&vCenter=true&width=750&lines=%F0%9F%9B%A1%EF%B8%8F+Learn+%E2%80%A2+Analyze+%E2%80%A2+Defend+%E2%80%A2+Secure;%F0%9F%94%8D+Wireshark+%2B+Nmap+%2B+Threat+Detection;%F0%9F%92%BB+Building+clean%2C+secure+web+experiences;%F0%9F%94%B5+Aspiring+Blue+Team+%2F+SOC+Analyst" />

<br/>

<a href="https://www.linkedin.com/in/sajankumaran-mu"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
<a href="https://sajankumaran.github.io/My_portfolio/"><img src="https://img.shields.io/badge/Portfolio-00FF9D?style=for-the-badge&logo=googlechrome&logoColor=black"/></a>
<a href="mailto:musajankumaran@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>


<img src="https://komarev.com/ghpvc/?username=sajankumaran-mu&label=Profile%20Views&color=00ff9d&style=for-the-badge"/>

</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f2027,100:00ff9d&height=3&width=100%"/>

<!-- ===================== ABOUT ===================== -->

<h2 align="center">🧑‍💻 WHO AM I?</h2>
<div align="center">

```text
┌────────────────────────────────────────────────────────────┐
│                                                            │
│   👋 Hi, I'm Sajan Kumaran                                 │
│                                                            │
│   🎓 Computer Science Engineering Student                  │
│   🛡️  Cybersecurity Enthusiast                            │
│   🔵 Future Blue Team Operator                             │
│   🌐 Web Development Experience                           │
│   🔎 Network Security & Threat Detection                   │
│                                                            │
│   I enjoy understanding how systems work, identifying       │
│   vulnerabilities and building practical security          │
│   solutions.                                                │
│                                                            │
└────────────────────────────────────────────────────────────┘
```
</div>

I'm a **Computer Science student** with a strong interest in **Cybersecurity, Network Security, Web Application Security, and Threat Detection**.

I enjoy exploring security tools, analyzing network activity, understanding vulnerabilities, and developing practical technical solutions. My current goal is to build strong expertise in **defensive cybersecurity and Blue Team operations**.

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f2027,100:00ff9d&height=3&width=100%"/>

<!-- ===================== CYBERSECURITY FOCUS ===================== -->

<h2 align="center">🛡️ CYBERSECURITY FOCUS</h2>

<div align="center">

```text
             ┌────────────────────────┐
             │    NETWORK SECURITY    │
             └────────────┬───────────┘
                ↓  
             ┌────────────────────────┐
             │    THREAT DETECTION    │
             └────────────┬───────────┘
              ↓
             ┌────────────────────────┐
             │ VULNERABILITY ANALYSIS │
             └────────────┬───────────┘
                             ↓               
             ┌────────────────────────┐
             │  SECURITY MONITORING   │
             └────────────┬───────────┘
              ↓
             ┌────────────────────────┐
             │   INCIDENT RESPONSE    │
             └────────────┬───────────┘
              ↓
             ┌────────────────────────┐
             │     🔵 BLUE TEAM       │
             └────────────────────────┘
```

</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f2027,100:00ff9d&height=3&width=100%"/>

<!-- ===================== TECH STACK ===================== -->

<h2 align="center">⚡ TECH STACK</h2>

<h3 align="center">💻 Programming & Development</h3>
<p align="center">
<img src="https://skillicons.dev/icons?i=python,java,c,html,css,js"/>
</p>

<h3 align="center">🗄️ Databases</h3>
<p align="center">
<img src="https://skillicons.dev/icons?i=mysql,mongodb"/>
</p>

<h3 align="center">🛡️ Cybersecurity & Networking</h3>
<p align="center">
<img src="https://skillicons.dev/icons?i=linux,kali,bash"/>
<br><br>
<img src="https://img.shields.io/badge/Wireshark-1679A7?style=for-the-badge&logo=wireshark&logoColor=white"/>
<img src="https://img.shields.io/badge/Nmap-00457C?style=for-the-badge&logoColor=white"/>
<img src="https://img.shields.io/badge/Web%20App%20Security-111111?style=for-the-badge&logo=security&logoColor=00FF9D"/>
</p>

<h3 align="center">🧰 Tools & Platforms</h3>
<p align="center">
<img src="https://skillicons.dev/icons?i=git,github,vscode"/>
</p>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f2027,100:00ff9d&height=3&width=100%"/>

<!-- ===================== SKILL MATRIX ===================== -->

<h2 align="center">📊 SKILL MATRIX</h2>

<div align="center">

| Domain | Technologies / Skills |
|:---:|:---|
| 🐍 Programming | Python • Java • C |
| 🌐 Web | HTML • CSS • JavaScript |
| 🗄️ Database | MySQL • MongoDB • SQL |
| 🛡️ Cybersecurity | Network Security • Vulnerability Analysis • Web Security |
| 🔍 Security Tools | Wireshark • Nmap |
| 📊 Data | Data Visualization • Data Analysis |
| 🔧 Tools | Git • GitHub • VS Code |
| 🐧 OS | Linux • Kali Linux |

</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f2027,100:00ff9d&height=3&width=100%"/>

<!-- ===================== EXPERIENCE ===================== -->

<h2 align="center">💼 EXPERIENCE</h2>

<table>
<tr>
<td width="50%" valign="top">

### 🛡️ Cybersecurity Intern
**EduTantr** · Bangalore, Karnataka
`Dec 2025 → Mar 2026`

- 🔎 Used **Wireshark & Nmap** for practical cybersecurity activities
- 🌐 Monitored network activity for anomalies
- 🧩 Identified and reported potential system vulnerabilities
- 🛡️ Built foundational knowledge in network security
- 🚨 Explored threat-detection techniques

</td>
<td width="50%" valign="top">

### 🌐 Web Development Intern
**CodeBind Technologies** · Chennai, Tamil Nadu
`Jun 2025 → Jul 2025`

- 💻 Developed & modified web content using **HTML & CSS**
- 🎨 Improved website layouts and accessibility
- 📱 Created simple, responsive web pages
- ⚙️ Applied fundamental web development concepts

</td>
</tr>
</table>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f2027,100:00ff9d&height=3&width=100%"/>

<!-- ===================== PROJECTS ===================== -->

<h2 align="center">🚀 FEATURED PROJECTS</h2>

---

<div align="center">

### 🔐 Image Steganography for Secure Data Hiding
`Dec 2024 → Jan 2025`

</div>

A security-focused project designed to hide confidential information inside digital images using the **Least Significant Bit (LSB)** steganography technique.
<div align="center">
```text
🖼️ Input Image
      ↓
🔐 Secret Data
      ↓
🧩 LSB Encoding
      ↓
📦 Hidden Information
      ↓
🔓 Decoding & Extraction
```

**Highlights**
- 🔐 Secure information hiding
- 🖼️ Digital image-based data embedding
- 🧩 LSB steganography technique
- 🔓 Decoding mechanism to retrieve hidden information
- 🛡️ Focus on privacy and secure communication

`Concept: Steganography` `Focus: Data Security` `Technique: LSB Encoding`
</div>
---

<div align="center">

### 📚 Learning Management System (LMS)

</div>

A web-based **Learning Management System** built to provide a centralized platform for managing courses, learning resources, and student activity.
<div align="center">
```text
👨‍🎓 Student                      👨‍🏫 Instructor
   │                                │
   ├── 📚 Browse Courses            ├── ➕ Create Courses
   ├── 📖 Access Materials          ├── 📤 Upload Materials
   ├── 📝 View Assignments          ├── 📝 Manage Assignments
   └── 📊 Track Progress            └── 👥 Manage Students
```

**Highlights**
- 📚 Course and learning-material management
- 👨‍🎓 Student-focused learning interface
- 👨‍🏫 Instructor / course management functionality
- 📝 Assignment management
- 📊 Learning progress tracking
- 🌐 Responsive web-based interface
- 🔐 Structured user and course management

`HTML` `CSS` `JavaScript` `SQL` `Web Development`
</div>
---

<div align="center">

### 🌐 Web Development Projects

</div>

Responsive web pages built with a focus on **usability, accessibility, and clean layouts**.

- 🎨 Responsive UI design
- 🌐 Front-end development
- 📱 Mobile-friendly layouts
- ⚡ User experience improvements
- 🧩 Web content development

<div align="center">

### 🔭 More Projects Coming Soon...
`Cybersecurity` • `Network Security` • `Web Security` • `Automation` • `AI`

</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f2027,100:00ff9d&height=3&width=100%"/>

<!-- ===================== CERTIFICATIONS ===================== -->

<h2 align="center">📜 CERTIFICATIONS</h2>
<div align="center">

```text
🏆 NoviTech Bootcamps (2024)
   ├── Data Analytics
   ├── MERN Stack
   └── Virtual Mouse with OpenCV

🏆 Wadhwani Foundation (2026)
   └── Interpersonal & Soft Skills

🏆 Coursera (2026)
   ├── Game Development with Scratch
   └── Computer Vision with Azure Cognitive Services

🏆 CodeBind Trainings (2025)
   ├── Web Development Implant Training
   ├── AI Workshop
   ├── Online LMS Project
   └── Corporate Aptitude Training

🏆 NPTEL (2026)
   └── Programming in Java

🏆 TESTYANTRA (2026)
   └── Problem Solving using Java
```
</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f2027,100:00ff9d&height=3&width=100%"/>

<!-- ===================== CURRENTLY LEARNING ===================== -->

<h2 align="center">📚 CURRENTLY LEARNING</h2>

<div align="center">

| 🔐 Cybersecurity | 🌐 Security | ☁️ Future Focus |
|:---:|:---:|:---:|
| Network Defense | Web Application Security | Cloud Security |
| Threat Detection | Security Monitoring | Security Automation |
| Vulnerability Analysis | Incident Response | SOC Operations |
| Linux Security | Ethical Hacking | Blue Team |

</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f2027,100:00ff9d&height=3&width=100%"/>

<!-- ===================== 2026 GOALS ===================== -->

<h2 align="center">🎯 2026 GOALS</h2>

```text
[████████████████░░░░]  Strengthen Cybersecurity Skills
[██████████████░░░░░░]  Master Network Security
[████████████░░░░░░░░]  Build Security Projects
[██████████░░░░░░░░░░]  Learn SOC & SIEM Tools
[████████░░░░░░░░░░░░]  Improve Linux Security Skills
[██████░░░░░░░░░░░░░░]  Earn Cybersecurity Certifications
```

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f2027,100:00ff9d&height=3&width=100%"/>

<!-- ===================== GITHUB ANALYTICS ===================== -->

<h2 align="center">📈 GITHUB ANALYTICS</h2>

<div align="center">



<br>

<img src="https://streak-stats.demolab.com?user=sajankumaran&theme=chartreuse-dark&hide_border=true"/>

<br><br>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=sajankumaran&theme=react-dark&hide_border=true&bg_color=0f2027&color=00ff9d&line=00ff9d&point=ffffff" width="100%"/>

</div>

<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f2027,100:00ff9d&height=3&width=100%"/>

<!-- ===================== CONTRIBUTION SNAKE ===================== -->


<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f2027,100:00ff9d&height=3&width=100%"/>

<!-- ===================== HACKERRANK ===================== --> <h2 align="center">🏅 HACKERRANK PROFILE</h2> <div align="center"> <a href="https://www.hackerrank.com/profile/musajankumaran" target="_blank"> <img src="https://img.shields.io/badge/HackerRank-View%20Profile-2EC866?style=for-the-badge&logo=hackerrank&logoColor=white"/> </a>

<br><br>

<img src="https://img.shields.io/badge/Problem%20Solving-Practice-00ff9d?style=for-the-badge&logo=hackerrank&logoColor=black"/> <img src="https://img.shields.io/badge/Java-Skill%20Badge-007396?style=for-the-badge&logo=hackerrank&logoColor=white"/> <img src="https://img.shields.io/badge/SQL-Skill%20Badge-4479A1?style=for-the-badge&logo=hackerrank&logoColor=white"/> </div> <img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f2027,100:00ff9d&height=3&width=100%"/> <!--

<!-- ===================== CONNECT ===================== -->

<h2 align="center">🌐 CONNECT WITH ME</h2>

<p align="center">
<a href="https://www.linkedin.com/in/sajankumaran-mu"><img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
<a href="https://sajankumaran.github.io/My_portfolio/"><img src="https://img.shields.io/badge/Portfolio-Visit-00FF9D?style=for-the-badge&logo=google-chrome&logoColor=black"/></a>
<a href="mailto:musajankumaran@gmail.com"><img src="https://img.shields.io/badge/Email-Contact-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>
</p>

<!-- ===================== FOOTER ===================== -->

<div align="center">

### 🛡️ `LEARN • ANALYZE • DEFEND • SECURE`

<br>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00ff9d,50:203a43,100:0f2027&height=120&section=footer"/>

</div>
